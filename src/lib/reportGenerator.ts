...
export async function generatePDFReport(data: ReportData) {
    ...
    allWilayah.forEach((wilayah, index) => {
        ...
        // --- 2. TABEL IBU (REVISI: Tambah Kolom Nama Bidan) ---
        if (pregnancyByWilayah[wilayah] && pregnancyByWilayah[wilayah].length > 0) {
            ...
            autoTable(doc, {
                startY: finalY + 5,
                head: [['NO', 'NAMA IBU', 'NAMA BIDAN', 'STATUS', 'USIA HAMIL', 'HPL', 'FE', 'KELOR', 'PERSALINAN', 'KONDISI']],
                body: pregnancyByWilayah[wilayah].map((p: any, i: number) => [
                    i + 1,
                    p.nama.toUpperCase(),
                    p.namaBidan, // <--- Kolom Baru
                    p.status,
                    p.usiaHamil,
                    p.hpl,
                    p.feProgress,
...
export function generateExcelReport(data: ReportData) {
    ...
    allWilayah.forEach((wilayah) => {
        ...
        // --- REVISI EXCEL: Tambah Kolom Nama Bidan ---
        rows.push([], ['--- DATA IBU HAMIL & BERSALIN ---']);
        rows.push(['NO', 'NAMA IBU', 'NAMA BIDAN', 'STATUS', 'USIA HAMIL', 'ESTIMASI HPL', 'PROG. FE', 'PROG. KELOR', 'CARA LAHIR', 'KONDISI LAHIR']);

        (pregnancyByWilayah[wilayah] || []).forEach((p: any, i: number) => {
            rows.push([i + 1, p.nama, p.namaBidan, p.status, p.usiaHamil, p.hpl, p.feProgress, p.kelorProgress, p.caraLahir, p.kondisiLahir]);
        });

        const ws = XLSX.utils.aoa_to_sheet(rows);

        // Atur Lebar Kolom (Sesuaikan dengan jumlah kolom)
        ws['!cols'] = [
            { wch: 5 }, { wch: 25 }, { wch: 20 }, { wch: 8 }, { wch: 15 },
